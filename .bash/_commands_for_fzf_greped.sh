...
git checkout origin/develop -- .     # tip of origin/develop ▪ git detach HEAD
...
git diff commit1 commit2 -- '*.c' | grep '^@@'  ▪ see functions
git stash  ▪ TODO!
...
git branch --sort=-committerdate            ▪ sort by last commit date?
...
./scripts/clang-format/format.sh --apply                         ▪ in the docker
...
scp wshabso@ilks-dockerpool:/work/wshabso/  ./  #
